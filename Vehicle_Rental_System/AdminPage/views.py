...
from rental.models import Maintenance, Vehicle  # Adjust if your models are in a different app
import datetime
...
def admin_payments_view(request): 
    ...
    This view handles the maintenance dashboard.
    - On POST, it creates a new maintenance record using the ORM.
    - On GET, it fetches all necessary data for the dashboard using the ORM.
    """
    if not request.user.is_superuser:
        messages.error(request, "You do not have permission to view this page.")
        return redirect('home')
...
def payment_form_view(request, payment_id=None):
    ...
    return render(request, "admin/payment_form.html", context)
        ...
        # Use the ORM to get the related vehicle instance
...
def bookings_management_view(request):
    ...
    if search_query:
        ...
        # Use the ORM's .create() method to insert a new row into the database
    ...
    # --- Fetch Data for Display (Read/Aggregate Operations) ---
    ...
    if 'export' in request.GET and request.GET['export'] == 'csv':
        ...
        return response
    # ORM query to get all maintenance records, ordering by the most recent.
    # `select_related` is an optimization to fetch the related vehicle in a single query.
    all_maintenance_records = Maintenance.objects.select_related('vehicle').order_by('-maintenance_date')
    ...
    if request.method == 'POST' and 'bulk_action' in request.POST:
        ...
        if booking_ids and action:
            ...
            return redirect('bookings_management')
    # ORM query to filter for upcoming maintenance (e.g., scheduled in the next 30 days)
    thirty_days_from_now = timezone.now() + datetime.timedelta(days=30)
    upcoming_maintenance_list = all_maintenance_records.filter(
        status='Scheduled',
        maintenance_date__lte=thirty_days_from_now
    ).order_by('maintenance_date')[:5] # Limit to the next 5
    ...
    # ORM query to calculate total cost per vehicle.
    # This generates a `GROUP BY vehicle__vehicle_name` and `SUM(cost)` SQL query.
    cost_per_vehicle_summary = Maintenance.objects.values(
        'vehicle__vehicle_name'  # The field to group by
    ).annotate(
        total_cost=Sum('cost')   # The aggregation function
    ).order_by('-total_cost')    # Order by the highest cost
    ...
    # Data needed for the "Add New Record" form dropdowns
    all_vehicles_list = Vehicle.objects.all()
    maintenance_type_choices_list = Maintenance.MAINTENANCE_TYPE_CHOICES
    status_choices_list = Maintenance.STATUS_CHOICES
    ...
    # Bundle all the data into a context dictionary to pass to the template
    context = {
        ...
        'upcoming_maintenance': upcoming_maintenance_list,
        'cost_per_vehicle': cost_per_vehicle_summary,
        'all_vehicles': all_vehicles_list,
        'maintenance_type_choices': maintenance_type_choices_list,
